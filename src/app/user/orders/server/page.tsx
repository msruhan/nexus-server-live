...
import type { ServerOrderStatus } from '@prisma/client';
...
const STATUS_TABS: Array<{ key: string; label: string; statuses?: ServerOrderStatus[] }> = [
  { key: 'all', label: 'All' },
  { key: 'active', label: 'In flight', statuses: [OrderStatus.PENDING, OrderStatus.IN_PROCESS] as ServerOrderStatus[] },
  { key: 'success', label: 'Successful', statuses: [OrderStatus.SUCCESS] as ServerOrderStatus[] },
  { key: 'refunded', label: 'Refunded', statuses: [OrderStatus.REJECTED, OrderStatus.CANCELLED] as ServerOrderStatus[] },
];

export default async function UserServerOrdersPage({
  searchParams,
}: {
  ...
  const [rows, total] = await Promise.all([
    ...
    prisma.serverOrder.count({ where }),
  ]);
  ...
  return (
    <div>
      <PageHeader
        ...
        title={
          <>
            Order <span className="font-serif italic font-normal">Server</span>.
          </>
        }
        subtitle="Your server order history with status and result codes."
      />
      ...
      <div className="mb-6 flex flex-wrap gap-1 rounded-full border border-line bg-paper-50 p-1 text-sm">
        {STATUS_TABS.map((t) => {
          ...
          return (
            <Link
              key={t.key}
              href={t.key === 'all' ? '/user/orders/server' : `/user/orders/server?status=${t.key}`}
      ...
      {rows.length === 0 ? (
        <EmptyState
          title="No server orders yet"
          description="Choose a server service and submit your first order."
          action={
            <Link href="/user/services/server" className="inline-flex items-center gap-1 rounded-full bg-ink px-4 py-2 text-xs font-semibold text-paper">
        ...
        <div className="overflow-hidden rounded-xl border border-line bg-paper-50">
            ...
            <tbody>
              {rows.map((o) => (
                <tr key={o.id} className="border-b border-line last:border-0 hover:bg-paper-100">
                  ...
                  <td className="hidden px-4 py-3 font-mono text-xs text-ink-muted sm:table-cell">{o.email ?? '—'}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link
                      href={`/user/orders/${o.id}?type=server`}
          ...
          <div className="border-t border-line bg-paper-50 px-4 py-3">
            <ServerTablePagination
              ...
              className="mt-0"
              buildHref={(p) =>
                buildTablePageHref('/user/orders/server', {
