...
/** POST /api/admin/imei/server-boxes/bulk-delete */
export async function POST(req: Request) {
  ...
  try {
    ...
    const serviceCounts = await prisma.serverService.groupBy({
      by: ['boxId'],
      where: { boxId: { in: ids } },
      _count: { _all: true },
    });
    ...
    const blocked = new Map(
      serviceCounts.map((row) => [row.boxId, row._count._all] as const),
    );
    ...
    const result = await prisma.serverServiceBox.deleteMany({
    ...
    console.error('[ADMIN_SERVER_BOXES_BULK_DELETE]', e);
